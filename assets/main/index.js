...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function resetToBottomRight() {
        ...
        console.log('Widget position reset to bottom-right due to zoom change');
    }
    ...
    function detectZoomChange() {
        ...
        // Check for zoom changes via devicePixelRatio or visualViewport
        ...
        // Additional check for visual viewport changes (covers more zoom scenarios)
        if (visualViewport) {
    ...
    function setWidgetPosition(x, y) {
        // First check if zoom changed
        if (detectZoomChange()) {
            return; // Position was reset, don't apply the drag
        }
    ...
    window.addEventListener('resize', () => {
        setTimeout(detectZoomChange, 100); // Delay to ensure proper measurement
    });

    // Listen for wheel events with Ctrl key (zoom gestures)
    document.addEventListener('wheel', (e) => {
        if (e.ctrlKey) {
            setTimeout(detectZoomChange, 200); // Delay to allow zoom to complete
        }
    }, { passive: false });
    ...
    document.addEventListener('keydown', (e) => {
        if (e.ctrlKey && (e.key === '+' || e.key === '-' || e.key === '0')) {
            setTimeout(detectZoomChange, 200);
        }
    });

    // Mouse events
    header.addEventListener('mousedown', (e) => {
        if (e.target === toggleBtn) return; // Don't drag when clicking toggle button
